...
file = open('inputCheck.txt', "r", encoding="utf-8")
...
def check(num):
    if s[num].find(encods[num-1]) != -1:
        print("Столбец №%d верный" % num)
    else:
        print(s[num])
        print("В столбце №%d что-то не так, сравните (первая строка из input2.txt, вторая из таблицы)" % num)
